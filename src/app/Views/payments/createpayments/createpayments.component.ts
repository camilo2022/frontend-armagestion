...
export class CreatepaymentsComponent {
  ...
  onSubmit(){
    ...
    if(this.payments.campaign_id !== 0){
      Swal.fire({
        ...
        if(result.value){
          this.paymentsService.storePayments(this.payments).subscribe(
            (success: any) => {
                this.indexPayments();
                this.alertSwal('success', '¡Accion Exitosa!', 'Creacion del pago exitosa.')
            },
            error => {
              // Manejo de errores aquí
              let mensaje = 'Se encontraron errores:\n';
          ...
          this.alertSwal('warning', '¡Accion cancelada!', 'Creacion del pago cancelada.')
        }
      })
    }
  }
