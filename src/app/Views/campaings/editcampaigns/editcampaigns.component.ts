...
@Component({
  selector: 'app-editcampaigns',
  templateUrl: './editcampaigns.component.html',
  styleUrls: ['./editcampaigns.component.css']
})
export class EditcampaignsComponent {

  campaign: Campaigns = {
    id: history.state.campaign.id,
    csts_id: history.state.campaign.csts_id,
    camp_name: history.state.campaign.camp_name,
    camp_description: history.state.campaign.camp_description,
    user_id: history.state.campaign.user_id,
  }
  ...
  onSubmit(){
    if(this.campaign.user_id !== 0){
      Swal.fire({
        title: '¿Estas seguro?',
        text: "Confirmar si deseas actualizar los datos de la campaña ingresados.",
        icon: 'question',
        ...
        if(result.value){
          this.campaignsService.updateCampaigns(this.campaign, this.campaign.id).subscribe(
            (success: any) => {
                this.indexCampaigns();
                this.alertSwal('success', '¡Accion Exitosa!', 'Actualizacion de la campaña exitosa.')
            },
            error => {
              // Manejo de errores aquí
              let mensaje = 'Se encontraron errores:\n';
          ...
          this.alertSwal('warning', '¡Accion cancelada!', 'Actualizacion de la campaña cancelada.')
        }
      })
    }
  }
